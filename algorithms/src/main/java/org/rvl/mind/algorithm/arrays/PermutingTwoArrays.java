package org.rvl.mind.algorithm.arrays;
...
public class PermutingTwoArrays {
  ...
  public String twoArrays(int k, List<Integer> a, List<Integer> b) {
    ...
    for (int i = 0; i < a.size(); i++) {
      List<Integer> idxCandidates = new ArrayList();
      ...
      if (idxCandidates.isEmpty()) {
        return "NO";
      } else {
        swap(b, i, getMin(idxCandidates, b));
      }
    }
  ...
  private Integer getMin(List<Integer> idxCandidates, List<Integer> b) {
    int min = Integer.MAX_VALUE;
    int idxMin = Integer.MAX_VALUE;
    for (int i = 0; i < idxCandidates.size(); i++) {
      if (b.get(idxCandidates.get(i)) < min) {
        min = b.get(idxCandidates.get(i));
        idxMin = idxCandidates.get(i);
      }
    }
    return idxMin;
  }

  private static void swap(List<Integer> b, int currentIdx, int idxMin) {
    int tmp = b.get(currentIdx);
    b.set(currentIdx, b.get(idxMin));
    b.set(idxMin, tmp);
  }
